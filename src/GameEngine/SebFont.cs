...
namespace GameEngine.SebText.FontLoading
{
    ...
    static class GlythHelper{
        public static List<Vector2[]> CreateContoursWithImpliedPoints(FontData.GlyphData character, float scale)
        {
            ...
            for (int contourIndex = 0; contourIndex < contourCount; contourIndex++)
            {
                ...
                Span<FontData.Point> contourPoints = character.Points.AsSpan(startPointIndex, numPointsInContour);
                ...
                for (int i = 0; i < contourPoints.Length; i++)
                {
                    FontData.Point curr = contourPoints[(i + firstOnCurvePointIndex + 0) % contourPoints.Length];
                    FontData.Point next = contourPoints[(i + firstOnCurvePointIndex + 1) % contourPoints.Length];
